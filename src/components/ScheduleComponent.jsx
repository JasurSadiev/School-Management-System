...
import LessonConductForm from "./LessonConductForm";
import { span } from "framer-motion/client";

const ScheduleComponent = ({ lessons, studentInfo }) => {
	...
	const getLessonsForWeek = (weekDays) => {
		return lessons.filter((lesson) => {
			const lessonDate = new Date(lesson.fullDate);
						...
						<Box key={index}>
							...
							<Box sx={{ mt: 1 }}>
								{dayLessons.map((lesson, lessonIndex) => {
									const lessonEndTime = new Date(lesson.fullDate);
											...
											<Card sx={{ mb: 1 }}>
												<CardContent>
													<Typography variant='body2'>
														{new Date(lesson.fullDate).toLocaleTimeString([], {
															hour: "2-digit",
															minute: "2-digit",
														})}
													</Typography>
													<Typography variant='caption'>
														{`${studentInfo[lesson.studentId]} ${
															lesson.courseName
														}`}
													</Typography>
													<br />
